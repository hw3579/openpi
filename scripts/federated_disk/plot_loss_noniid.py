...
def plot_clients_loss(
    ...
    for cid, fp in selected:
        ...
        if not recs:
            continue
        # 如果设置了 max_rounds，则仅保留前 max_rounds 轮数据
        if max_rounds is not None:
            recs = [r for r in recs if r.round <= max_rounds]
            if not recs:
                continue
    ...
    plt.ylabel(metric)
    plt.title("FL-Non-IID Client Loss Curves")
    plt.legend()
...
def main() -> None:
    ...
    parser.add_argument("--max-rounds", type=int, default=None, help="只绘制前 N 轮 (默认: 不限制)")
