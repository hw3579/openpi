"""OpenPI Flower Client (Disk-based, Non-IID):
Per-client dataset via repo_id override loaded from pyproject.toml.

Goals
- Keep base TrainConfig unchanged, only replace cfg.data.repo_id using TOML
...
try:
    ...
    _cc_dir = os.environ.get(
        ...
        str(_ROOT / "cache" / "jax_persistent"),
    )
    pathlib.Path(_cc_dir).mkdir(parents=True, exist_ok=True)
    _jax_cc.initialize_cache(_cc_dir)
    print(f"[DiskClientNonIID] JAX persistent compilation cache: {_cc_dir}")
except Exception:
    pass
...
# Note: orbax is not needed in the Non-IID disk client path


# IIDSubsetDataset is unused in the Non-IID client; removed to reduce clutter
...
def _apply_model_params(loaded: dict, state: training_utils.TrainState) -> training_utils.TrainState:
    ...
    import flax.nnx as nnx  # local import to satisfy type checkers
...
def _get_repo_id_from_toml(config_name: str, client_id: int) -> str | None:
    """Read pyproject.toml to fetch per-client repo_id for a given config.

    Layout expected:
    [tool.flwr.app.noniid.<config_name>]
    repo_id_by_client = ["repo_for_client_0", "repo_for_client_1", ...]
    """
    ...
    try:
        ...
        if isinstance(cfgsec, dict):
            arr = cfgsec.get("repo_id_by_client")
            if isinstance(arr, list) and 0 <= client_id < len(arr):
                val = arr[client_id]
                if isinstance(val, str) and val.strip():
                    return val
        return None
    except Exception as e:  # pragma: no cover
        print(f"[DiskClientNonIID {client_id}] Failed to read TOML noniid mapping: {e}")
        return None
...
class OpenPIFlowerDiskClientNonIID(NumPyClient):
    def __init__(
        self,
        config_name: str,
        total_clients: int,
        ...
        # Replace top-level training fields
        self.cfg = dataclasses.replace(
            self.cfg,
            exp_name=f"flwr_disk_noniid_client_{client_id}",
            wandb_enabled=False,
        ...
        # Non-IID: override data.repo_id per client from TOML if present
        override_repo = _get_repo_id_from_toml(self.cfg.name, client_id)
        if override_repo:
            try:
                new_data = dataclasses.replace(self.cfg.data, repo_id=override_repo)
                self.cfg = dataclasses.replace(self.cfg, data=new_data)
                print(f"[DiskClientNonIID {client_id}] Overrode data.repo_id -> {override_repo}")
            except Exception as e:
                print(f"[DiskClientNonIID {client_id}] Failed to override repo_id: {e}")
    ...
    def _lazy_init(self):
        if self.initialized:
            return
        try:
            print(f"[DiskClientNonIID {self.client_id}] JAX backend: {jax.default_backend()}, devices: {jax.devices()}")
        except Exception:
            pass
        ...
        # Build dataset directly from this client's repo_id (non-IID across clients)
        ...
        dataset = dl.transform_dataset(base_dataset, data_config, skip_norm_stats=False)
    ...
    def fit(self, parameters, config):
        # Best-effort pre-fit cleanup if previous round leaked
        try:
            gc.collect(); jax.clear_caches()
        except Exception:
            pass
        rnd = int(config.get("server_round", 0))
        # Fast-path: if this round output already exists, skip training to support resume mid-round
        try:
            ...
            if out_path.exists():
                ...
                print(f"[DiskClientNonIID {self.client_id}] Skip training for round {rnd}: reuse {out_path}")
        ...
        if not gpath:
            print(f"[DiskClientNonIID {self.client_id}] No params_path provided; skip")
            return [], 0, {"error": "no_params_path"}
        # Apply global parameters
        try:
            self.state = _apply_params_from(gpath, self.state)
        except Exception as e:
            print(f"[DiskClientNonIID {self.client_id}] Failed to apply params from {gpath}: {e}")
        ...
        # Write sidecar meta with examples
        try:
            meta = {
                "round": int(rnd),
                "client_id": int(self.client_id),
            ...
            pass
        print(f"[DiskClientNonIID {self.client_id}] Saved trained params -> {path_str}")
        ...
        # Post-fit cleanup: aggressively drop device references and caches
        try:
            def _block(x):
                try:
    ...
    def evaluate(self, parameters, config):
        print(f"[DiskClientNonIID {self.client_id}] Skipping evaluation")
...
if __name__ == "__main__":
    print("OpenPI Disk-based Flower Client (Non-IID) - use 'flwr run' to start")
