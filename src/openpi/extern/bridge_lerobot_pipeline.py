...
import numpy as np
import jax.numpy as jnp  # 添加JAX导入
import logging
import math
...
class BridgeLeRobotIterableDataset(IterableDataset):
    ...
    def __init__(
        ...
        use_jax: bool = True,  # 新增：是否使用JAX数组
    ):
        """
        Args:
        ...
        try:
            ...
            for i, episode in enumerate(it):
                ...
                for step in episode["steps"]:
                    # 生成单个样本，使用JAX数组
                    yield {
                        "image":    self._convert_array(step["observation"]["image_0"]),
                        "state":    self._convert_array(step["observation"]["state"]),
                        "actions":  self._convert_array(step["action"]),
...
def make_dataloader(
    ...
    # 根据数组类型选择不同的collate函数
    if use_jax:
        def jax_collate_fn(batch):
            return {
                "image": jnp.stack([b["image"] for b in batch]),
                "state": jnp.stack([b["state"] for b in batch]),
                "actions": jnp.stack([b["actions"] for b in batch]),
    ...
    return DataLoader(
        ...
        pin_memory=pin_memory and not use_jax,  # JAX不需要pin_memory
    ...
    def _convert_to_openpi_format(self, item):
        ...
        if self.use_jax:
            return {
                "observation": {
                    "images": {
                        "top": np.array(item["image"]),  # 转回numpy用于兼容性
                    },
                    "state": np.array(item["state"]),
                },
                "action": np.array(item["actions"]),
                "prompt": item["task"],
            }
        else:
            return {
